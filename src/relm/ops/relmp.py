"""Stable-ABI relational message-passing operator wrappers."""
...
def _fallback_mode() -> str:
    raw = os.getenv("RELM_RELMP_FALLBACK", "python").strip().lower()
...
def _ensure_loaded() -> None:
    global _LIB_LOADED, _LIB_LOAD_ERROR
    if _LIB_LOADED:
        return
    if torch is None:
        raise ModuleNotFoundError(
            "relm.ops.relmp requires torch."
    ...
    for candidate in _candidate_libraries():
        try:
            ...
            namespace = getattr(torch.ops, "relm_relmp", None)
            if namespace is None or any(
                not hasattr(namespace, op_name) for op_name in _REQUIRED_NAMESPACE_OPS
            ):
                raise RuntimeError(
                    f"Loaded {candidate} but torch.ops.relm_relmp is incomplete."
                )
            _LIB_LOADED = True
    ...
    if last_error is not None:
        raise RuntimeError(
            "Failed to load relm_relmp custom op library."
        ) from last_error
    raise FileNotFoundError(
        "Could not find relm_relmp custom op library in the relm package directory."
    )
...
def assert_runtime_compat() -> Dict[str, str]:
    ...
    if torch is None:
        raise ModuleNotFoundError("torch is required for relmp compatibility checks.")
    _ensure_loaded()
    if _BUILD_INFO_CACHE is None:
        info_raw = torch.ops.relm_relmp.build_info()
        _BUILD_INFO_CACHE = _parse_build_info(info_raw)

    info = _BUILD_INFO_CACHE
    build_torch = info.get("build_torch")
    if not build_torch:
        raise RuntimeError("relmp build metadata is missing build_torch.")
...
def _should_use_custom(op_name: str) -> bool:
    if not _env_bool("RELM_RELMP_ENABLE", True):
        return False
    try:
        _ensure_loaded()
        assert_runtime_compat()
        ...
        if _fallback_mode() == "error":
            raise RuntimeError(f"Custom relmp op {op_name} is unavailable.") from exc
        return False
...
if torch is not None:
    ...
    class _FanoutScatterFunction(torch.autograd.Function):
        ...
        def forward(
            ...
            return torch.ops.relm_relmp.fanout_scatter(
        ...
        def backward(
            ...
            grad_x = torch.ops.relm_relmp.fanout_scatter_backward(
    ...
    class _FaninReduceSumFunction(torch.autograd.Function):
        ...
        def forward(
            ...
            return torch.ops.relm_relmp.fanin_reduce(
        ...
        def backward(
            ...
            grad_rel = torch.ops.relm_relmp.fanin_reduce_sum_backward(
