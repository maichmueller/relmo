...
def test_flat_relational_layer_matches_staged_two_layer_silu_then_postnorm_program() -> None:
    model, relation_slice = _make_family_model(
        torch.nn.Sequential(
            TwoLayerPointwiseRelationMLP(8, 16, activation="silu"),
...
def test_manual_postnorm_program_reference_matches_loop_reference_forward_and_gradients() -> None:
    def loop_reference(
        ...
        for gid, row_size_t in enumerate(row_sizes):
            ...
            stage2 = torch.nn.functional.layer_norm(
                stage2,
                (int(stage2.size(-1)),),
                weight=ln_weight_stack[gid],
                bias=ln_bias_stack[gid],
                eps=float(ln_eps),
            )
            out[cursor : cursor + row_count] = rows + stage2
            cursor += row_count
    ...
    out = execute_program_two_layer_silu_then_postnorm_two_layer_silu_reference(
        packed_rows,
        row_sizes,
        *params,
        ln_eps,
...
def test_flat_relational_layer_fused_staged_two_layer_silu_then_postnorm_program_matches_direct() -> None:
    relation_dict = {"rel_a": 2, "rel_b": 2, "rel_c": 1}
    torch.manual_seed(0)
    model = FlatRelationalGNN(
        embedding_size=4,
...
@pytest.mark.skipif(not torch.cuda.is_available(), reason="CUDA required")
def test_flat_relational_layer_fused_staged_two_layer_silu_then_postnorm_program_cuda_custom_backward_matches_direct() -> None:
    if not relm_mp_module.available():
        pytest.skip("Custom mp ops are unavailable.")
